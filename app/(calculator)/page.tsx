...
const CalculatorForm = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const { coordinates, orderMinimumNoSurcharge, basePrice, distanceRanges } = venueInfo;

      // Calculate the delivery distance (straight line)
      const [venueLongitude, venueLatitude] = coordinates;
      const distance = haversineDistance(
        parseFloat(userLatitude), parseFloat(userLongitude), // User coordinate
      ...
      const smallOrderSurcharge = Math.max(orderMinimumNoSurcharge - cartValueInCents, 0);
        ...
        <div className={styles.priceBreakdown}>
          ...
          <table>
            <tbody>
              <tr>
                <td>Cart Value:</td>
                <td data-test-id="priceCartValue">{priceBreakdown.cartValue}</td>
              </tr>
              <tr>
                <td>Small Order Surcharge:</td>
                <td data-test-id="smallOrderSurcharge">{priceBreakdown.smallOrderSurcharge}</td>
              </tr>
              <tr>
                <td>Delivery Fee:</td>
              ...
              <tr>
                ...
                <td data-test-id="deliveryDistance">{priceBreakdown.deliveryDistance}</td>
              </tr>
              <tr>
                <td><strong>Total Price:</strong></td>
                <td data-test-id="totalPrice"><strong>{priceBreakdown.totalPrice}</strong></td>
              </tr>
            </tbody>
          </table>
        </div>
